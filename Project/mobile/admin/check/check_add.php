...
if (isset($_POST['action']) && $_POST['action'] == 'product') {
	$product = new Product();
	if (
		isset($_POST['name']) && isset($_POST['manu_id']) && isset($_POST['type_id'])
		...
		if (isset($_FILES['fileUpload'])) {
			...
			if (file_exists($target_file)) {
				echo "Image already exists .<a href='javascript: history.go(-1)'>Back</a>";
				$uploadOk = 0;
				die();
			} else {
				$uploadOk = 1;
			...
			if ($fileUpload['type'] == 'image/jpeg' || $fileUpload['type'] == 'image/jpg' || $fileUpload['type'] == 'image/png') {
				...
				$uploadOk = 0;
				echo "only JPG, JPEG, PNG & GIF files are allowed .<a href='javascript: history.go(-1)'>Back</a>";
				die();
			}
			//check file size
			...
			if ($uploadOk == 0) {
				...
				echo "Upload succesfully!!! .<a href='javascript: history.go(-1)'>Back</a>";
			}
		} else {
			echo "No images has been selected .<a href='javascript: history.go(-1)'>Back</a>";
		}
	}
}
